...
class SyncService:
    ...
    def onMessage(self, data: dict):
        ...
        if data['type'] == 'record':
            ...
            ServDev_id = data['dev_id']
            ServRec_Data = data['data']
            record = {'timestamp': Server_Time, 'dev_id': ServDev_id, 'data': ServRec_Data}
            self.syncSerRec.append(record)
            return None
            ...
            UpdatedRec = {
                'type': 'update', 'from': len(self.syncSerRec), 'dev_id': data['dev_id'],
                'data': self.syncSerRec['data']
            }
            return UpdatedRec
...
"""According to what I understood the data has been received for all devices. 
SyncService has an empty __init__ method and an onMessage method that raises a NotImplementedError.
